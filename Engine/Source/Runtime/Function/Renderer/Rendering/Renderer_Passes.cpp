...
namespace LitchiRuntime
{
    namespace
    {
        ...
        void draw_renderable(RHI_CommandList* cmd_list, RHI_PipelineState& pso, RendererPath* rendererPath,
                             GameObject* entity, uint32_t array_index = 0)
        {
            ...
            RHI_VertexBuffer* vertex_buffer = skinned_mesh_renderer != nullptr
                                                  ? mesh->GetVertexBufferWithBone()
            ...
            cmd_list->DrawIndexed(renderable->GetIndexCount(), renderable->GetIndexOffset(),
    ...
    void Renderer::SetStandardResources(RHI_CommandList* cmd_list)
    {
        ...
        cmd_list->SetConstantBuffer(Renderer_BindingsCb::rendererPath,
                                    GetConstantBuffer(Renderer_ConstantBuffer::RendererPath));
    ...
    void Renderer::Pass_ShadowMaps(RHI_CommandList* cmd_list, RendererPath* rendererPath,
                                   const bool is_transparent_pass)
    {
        ...
        pso.blend_state = is_transparent_pass
                              ? GetBlendState(Renderer_BlendState::Alpha).get()
                              : GetBlendState(Renderer_BlendState::Off).get();
        pso.depth_stencil_state = is_transparent_pass
                                      ? GetDepthStencilState(Renderer_DepthStencilState::Read).get()
                                      : GetDepthStencilState(Renderer_DepthStencilState::ReadWrite).get();
        ...
        for (uint32_t light_index = 0; light_index < lightCount; light_index++)
        {
            ...
            for (uint32_t array_index = 0; array_index < lightData.m_shadow_count; array_index++)
            {
                ...
                if (!entities.empty())
                {
                    ...
                    int64_t index_end = !is_transparent_pass
                                            ? rendererPath->GetMeshIndexTransparent()
                                            : static_cast<int64_t>(rendererPath->GetRenderables()[Renderer_Entity::Mesh]
                                                .size());
                    ...
                    for (int i = index_start; i < index_end; i++)
                    {
                        ...
                        m_cb_pass_cpu.set_light(static_cast<float>(array_index), static_cast<float>(light_index),
                                                lightCount);
                                    ...
                                    const bool is_transparent_pass)
    {
        ...
        RHI_RasterizerState* rasterizer_state = is_wireframe
                                                    ? GetRasterizerState(Renderer_RasterizerState::Wireframe_cull_none).
                                                    get()
                                                    : GetRasterizerState(Renderer_RasterizerState::Solid_cull_back).
                                                    get();
        ...
        pso.clear_color[0] = is_transparent_pass ? rhi_color_load : rhi_color_dont_care;
        pso.primitive_topology = RHI_PrimitiveTopology::TriangleList;
        ...
        int64_t index_end = !is_transparent_pass
                                ? rendererPath->GetMeshIndexTransparent()
                                : static_cast<int64_t>(rendererPath->GetRenderables()[Renderer_Entity::Mesh].size());
        const auto& rendererLightGroup = rendererPath->GetRendererLightGroup();
        const auto& lightCount = rendererLightGroup.GetLightCount();
        for (int64_t i = index_start; i < index_end; i++)
        {
            ...
            cmd_list->SetPipelineState(pso, needBeginRenderPass);
            // cmd_list->ClearPipelineStateRenderTargets(pso);
            needBeginRenderPass = false;
            EASY_END_BLOCK
            ...
            {
                // just main light
                cmd_list->SetTexture(Renderer_BindingsSrv::light_depth, rendererLightGroup.m_texture_depth);
                m_cb_pass_cpu.set_light(static_cast<float>(0), static_cast<float>(0), lightCount);
                cmd_list->SetStructuredBuffer(Renderer_BindingsUav::sb_lights,
                                              rendererLightGroup.m_light_structure_buffer);
            }
    ...
    void Renderer::Pass_GridPass(RHI_CommandList* cmd_list, RendererPath* rendererPath)
    {
        ...
        // set transform
        {
            ...
            m_cb_pass_cpu.transform = Matrix::CreateScale(Vector3(1000.0f, 1.0f, 1000.0f)) * Matrix::CreateTranslation(
                translation);
    ...
    void Renderer::Pass_LinesPass(RHI_CommandList* cmd_list, RendererPath* rendererPath)
    {
        ...
        // -1 because +1 will make it go to size / 2

        cmd_list->EndTimeblock();
    }

    void Renderer::Pass_IconPass(RHI_CommandList* cmd_list, RendererPath* rendererPath,
            ...
            if (auto light = entity->GetComponent<Light>())
            {
                ...
                if (light->GetLightType() == LightType::Directional)
                    texture = GetStandardTexture(
                        Renderer_StandardTexture::Gizmo_light_directional).get();
                else if (light->GetLightType() == LightType::Point)
                    texture = GetStandardTexture(
                        Renderer_StandardTexture::Gizmo_light_point).get();
                else if (light->GetLightType() == LightType::Spot)
                    texture = GetStandardTexture(
                        Renderer_StandardTexture::Gizmo_light_spot).get();
            }
    ...
    void Renderer::Pass_SelectedAssetViewResourcePass(RHI_CommandList* cmd_list, RendererPath* rendererPath)
    {
        ...
        if (selectMaterial != nullptr && selectMaterial->GetShader()->m_vertex_shader->GetInputLayout()->GetVertexType()
            != RHI_Vertex_Type::PosUvNorTan)
        {
            return;
        }
        ...
        int indexCount;
        Matrix transform = Matrix::CreateScale(1) * Matrix::CreateTranslation(Vector3::Zero) * Matrix::CreateRotation(
            Quaternion::Identity);
        std::shared_ptr<RHI_ConstantBuffer> selectedMeshBoneConstantBuffer;

        auto worldMatrix = Matrix::CreateScale(1) * Matrix::CreateTranslation(Vector3::Zero) * Matrix::CreateRotation(
            Quaternion::Identity);
        ...
        case SelectedResourceType_Mesh:
            m_vertex_buffer = selectedMesh->IsAnimationModel()
                                  ? selectedMesh->GetVertexBufferWithBone()
                                  : selectedMesh->GetVertexBuffer();
            m_index_buffer = selectedMesh->GetIndexBuffer();
            indexCount = selectedMesh->GetIndexCount();
        // transform = ;
            selectMaterial = selectedMesh->IsAnimationModel()
                                 ? m_default_standard_skin_material
                                 : m_default_standard_material;
        ...
        if (selectedResType == SelectedResourceType_Mesh)
        {
            ...
            if (subMeshArr.size() > 0)
            {
                ...
                {
                    cmd_list->DrawIndexed(subMesh.m_geometryIndexCount, subMesh.m_geometryIndexOffset,
                                          subMesh.m_geometryVertexOffset);
                }
            }
        }
        else
